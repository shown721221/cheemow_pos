  ...
  group('SearchFilterManager', () {
    ...
    final products = [
      ...
      Product(
        id: 'p3',
        barcode: '0003',
        name: '香港 Disney CookieAnn 坐姿 娃娃',
        price: 150,
        stock: 2,
      ),
      Product(
    ...
    test(
      'search: empty query returns empty list; basic name/barcode matching',
      () {
        expect(manager.search(products, ''), isEmpty);
        expect(
          manager.search(products, '0001').map((p) => p.id),
          containsAll(['p1']),
        );
        expect(
          manager.search(products, 'duffy').map((p) => p.id),
          containsAll(['p1']),
        );
      },
    );
